...
def SaveRequest (parent):
	...
	return { gtk.RESPONSE_CANCEL: 'CANCEL', gtk.RESPONSE_YES: 'YES', gtk.RESPONSE_NO: 'NO', gtk.RESPONSE_DELETE_EVENT: 'CANCEL' }[resp]
...
def CanOverwrite (parent, fileName):
	...
	return { gtk.RESPONSE_CANCEL: False, gtk.RESPONSE_YES: True, gtk.RESPONSE_DELETE_EVENT: False }[resp]

def getText (textView):
	buffer = textView.get_buffer()
	start, end = buffer.get_bounds()
	text = buffer.get_text(start=start,end=end).decode('utf-8')
	return text
...
def translateBuilder (builder):
	for obj in builder.get_objects():
		...
		elif type(obj) in (gtk.ImageMenuItem, gtk.TextView, gtk.TreeView, gtk.SeparatorMenuItem, gtk.VPaned, gtk.VBox, gtk.Menu, gtk.HPaned, gtk.MenuBar, gtk.ScrolledWindow, gtk.Statusbar, gtk.Entry, gtk.Table, gtk.HBox):
			pass
		else:
...
def doFind (start, textToFindEncoded, backward, ignoreCase):
	...
	if ignoreCase:
		...
		if backward:
			...
			text = buffer.get_text(end, start).decode('utf-8').lower()
			...
			text = buffer.get_text(start, end).decode('utf-8').lower()
			r = text.find(textToFind)
			if r >= 0:
